...
I18N = {
    "de": {
        ...
        "dlg_vc_redist_title": "VC++ Runtime fehlt",
        "dlg_vc_redist_msg": "Das Microsoft Visual C++ Redistributable (x64) wurde nicht gefunden. Soll es jetzt heruntergeladen und installiert werden?",
        "info_vc_redist_abort": "Das Microsoft Visual C++ Redistributable wird benötigt. Das Programm wird beendet.",
    ...
    "en": {
        ...
        "dlg_vc_redist_title": "VC++ runtime missing",
        "dlg_vc_redist_msg": "Microsoft Visual C++ Redistributable (x64) is missing. Download and install now?",
        "info_vc_redist_abort": "Microsoft Visual C++ Redistributable is required. The program will exit.",
...
if __name__ == "__main__":
    # Vor GUI-Start: VC++ Redistributable prüfen (Windows)
    if IS_WINDOWS and not _win_has_vc_redist():
        from tkinter import Tk, messagebox
        lang = detect_lang()
        S = I18N["de" if (lang or "").startswith("de") else "en"]
        _root = Tk(); _root.withdraw()
        if messagebox.askyesno(S["dlg_vc_redist_title"], S["dlg_vc_redist_msg"]):
            try:
                vc_url = "https://aka.ms/vs/17/release/vc_redist.x64.exe"
                from urllib.parse import urlsplit
                vc_exe = Path(tempfile.gettempdir()) / Path(urlsplit(vc_url).path).name
                download_file(vc_url, vc_exe, lambda *_: None)
                cmd = [str(vc_exe), "/install", "/quiet", "/norestart"]
                subprocess.run(" ".join(f'"{c}"' for c in cmd), shell=True, check=False)
            except Exception:
                pass
            if not _win_has_vc_redist():
                messagebox.showinfo(S["dlg_vc_redist_title"], S["info_vc_redist_abort"])
                sys.exit(1)
            # TODO: Installationsstatus in Einstellungen schreiben (Task 2)
        else:
            messagebox.showinfo(S["dlg_vc_redist_title"], S["info_vc_redist_abort"])
            sys.exit(1)
